...
**DeepSeek 不支持直接接收图像**——你无法把图片作为输入直接“发”给它。这也是本插件存在的意义：

- 你截图（`Win + Shift + S`）或复制一张图片；
- 插件读取**系统剪贴板**中的图片（不需要上传文件，也不需要图片附件入口）；
- 插件把图片交给你自己配置的视觉 API 识别，转成文字；
- DeepSeek 拿到文字结果后继续推理、回答或写代码。

简单说：**图片不是发给 DeepSeek 的，而是由插件代为“看”完后转述成文字**。

## 工作原理

```text
你截图/复制图片 ──▶ 插件读取剪贴板或本地路径 ──▶ deepseek-vision MCP 服务器 ──▶ 视觉 API
                                              (mcp-vision)                     │
Codex + DeepSeek ◀──────────── 文字识别结果 ◀───────────────────────────────────┘
```
...
7. **文档排版质检**：DOCX 交付前自动渲染并逐页检查文字裁剪、表格错位、缺字等问题。
